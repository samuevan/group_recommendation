...
if __name__ == '__main__':
    ...
    for fu in range(1,half_users,step_half):
        lu2 = min(fu+step_half-1,half_users)
        lu = min(fu+step_half-1,users[half_users])
        args.fu = fu
        args.lu = lu
        print('{}->{}->{}'.format(args.fu,args.lu,lu2))

        os.system('python save_distances.py --base {base} --fu {fu} --lu {lu} -o {out_dir} --simi_func {simi_func}&'.format(**vars(args)))

    print('half')
    first_2nd_half = args.lu+1
    step_2nd_half = int((len(users)-half_users)/(args.nump-halfp))
    for fu in range(first_2nd_half,len(users)+1,step_2nd_half+1):
        lu = min(fu+step_2nd_half,users[-1])
        #here we use len(users)+1 because the user_id will start from 1
        #therefore the last user_id should be len(users)+1
        lu2 = min(fu+step_2nd_half,len(users))
        args.fu = fu
        args.lu = lu
        print('{}->{}->{}'.format(args.fu,args.lu,lu2))
        os.system('python save_distances.py --base {base} --fu {fu} --lu {lu} -o {out_dir} --simi_func {simi_func}&'.format(**vars(args)))
